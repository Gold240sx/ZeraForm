...
```swift
dependencies: [
    .package(url: "https://github.com/yourusername/ZyraForm.git", from: "2.5.0")
]
```
...
**Version 2.5.0** - Current stable release. Added programmatic watch control with `stopWatching()` and `resumeWatching()` methods for `ZyraSync` and `SchemaBasedSync`. Perfect for multi-page apps to free up memory and network resources when views are not active. Users should not use versions less than 2.1.0.
