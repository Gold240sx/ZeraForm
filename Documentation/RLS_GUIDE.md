...
The new fluent API provides a more intuitive way to define RLS policies:
...
let userTable = ZyraTable(
    ...
    rlsPolicies: userTable.fluentRls()
        .who([.authenticated])
        .permissive()
...
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));

CREATE POLICY "users_authenticated_write" 
ON "users" AS PERMISSIVE FOR INSERT 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid))
WITH CHECK ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));

CREATE POLICY "users_authenticated_update" 
ON "users" AS PERMISSIVE FOR UPDATE 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid))
WITH CHECK ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));

CREATE POLICY "users_authenticated_delete" 
ON "users" AS PERMISSIVE FOR DELETE 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));
```
...
### RLSRole (New Fluent API)

Defines who can access (roles array):

- `.anonymous` - Anonymous/unauthenticated users (`auth.uid() IS NULL`)
- `.authenticated` - Authenticated users (`auth.uid() IS NOT NULL`)
- `.unauthenticated` - Same as anonymous
- `.admin` - Users with "admin" role (checks users table)
- `.superadmin` - Users with "superadmin" role (checks users table)

### RLSAccess (New Fluent API)
...
let personalTable = ZyraTable(
    ...
    rlsPolicies: personalTable.fluentRls(userIdColumn: "user_id")
        .who([.authenticated])
        .access([.read, .write, .update, .delete])
        .own()
        .build()
...
**Generated SQL:**
```sql
CREATE POLICY "personal_notes_authenticated_read" 
ON "personal_notes" AS PERMISSIVE FOR SELECT 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));

CREATE POLICY "personal_notes_authenticated_write" 
ON "personal_notes" AS PERMISSIVE FOR INSERT 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid))
WITH CHECK ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));

CREATE POLICY "personal_notes_authenticated_update" 
ON "personal_notes" AS PERMISSIVE FOR UPDATE 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid))
WITH CHECK ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));

CREATE POLICY "personal_notes_authenticated_delete" 
ON "personal_notes" AS PERMISSIVE FOR DELETE 
USING ((auth.uid() IS NOT NULL) AND (user_id::uuid = (auth.uid())::uuid));
```
...
let publicTable = ZyraTable(
    ...
    rlsPolicies: publicTable.fluentRls()
        .who([.anonymous, .authenticated])
        .access([.read])
        .match("true")
        .build()
)
...
**Generated SQL:**
```sql
CREATE POLICY "public_posts_anonymous_authenticated_read" 
ON "public_posts" AS PERMISSIVE FOR SELECT 
USING ((auth.uid() IS NULL OR auth.uid() IS NOT NULL) AND (true));
```
...
let postsTable = ZyraTable(
    ...
    rlsPolicies: postsTable.fluentRls(userIdColumn: "author_id")
        .who([.anonymous, .authenticated])
        .access([.read])
        .match("true")
        .build()
        +
        postsTable.fluentRls(userIdColumn: "author_id")
        .who([.authenticated])
        .access([.write, .update, .delete])
        .own()
        .build()
)
```
...
**Generated SQL:**
```sql
CREATE POLICY "posts_anonymous_authenticated_read" 
ON "posts" AS PERMISSIVE FOR SELECT 
USING ((auth.uid() IS NULL OR auth.uid() IS NOT NULL) AND (true));

CREATE POLICY "posts_authenticated_write" 
ON "posts" AS PERMISSIVE FOR INSERT 
USING ((auth.uid() IS NOT NULL) AND (author_id::uuid = (auth.uid())::uuid))
WITH CHECK ((auth.uid() IS NOT NULL) AND (author_id::uuid = (auth.uid())::uuid));
...
### Using Custom Match Expressions
...
let teamTable = ZyraTable(
    ...
    rlsPolicies: teamTable.fluentRls()
        .who([.authenticated])
        .access([.read, .write, .update, .delete])
        .match("""
            team_id IN (
...
let adminTable = ZyraTable(
    ...
    rlsPolicies: adminTable.fluentRls(usersTableName: "public.user_public")
        .who([.admin])
        .access([.read, .write, .update, .delete])
        .match("true")
        .build()
...
// Multiple roles
let multiRoleTable = ZyraTable(
    name: "settings",
    columns: [zf.text("value").notNull()],
    rlsPolicies: multiRoleTable.fluentRls()
...
**New Fluent API RBAC:**
- `.who([.admin])` - Users with "admin" role
- `.who([.superadmin])` - Users with "superadmin" role
- `.who([.admin, .superadmin])` - Multiple roles (OR logic)
- `.who([.authenticated, .admin])` - Authenticated users OR admins

**Legacy API RBAC Methods:**
...
### FluentRLSPolicyBuilder Methods (New API)

| Method | Description | Returns |
|--------|-------------|---------|
| `.who([RLSRole])` | Define who can access (roles array) | `FluentRLSPolicyBuilder` |
| `.permissive()` | Set policy type to permissive (default) | `FluentRLSPolicyBuilder` |
| `.restrictive()` | Set policy type to restrictive | `FluentRLSPolicyBuilder` |
| `.access([RLSAccess])` | Define access operations (read, write, update, delete) | `FluentRLSPolicyBuilder` |
| `.match(String)` | Custom SQL match expression | `FluentRLSPolicyBuilder` |
| `.own()` | Use default "own row" matching (user_id = auth.uid()) | `FluentRLSPolicyBuilder` |
| `.build()` | Build the RLS policies | `[RLSPolicy]` |

**Example:**
```swift
table.fluentRls(userIdColumn: "user_id", usersTableName: "public.user_public")
...
### RLSPolicyBuilder Methods (Legacy API)
...
| `rls(userIdColumn:usersTableName:roleColumn:isOnlineColumn:)` | Get legacy RLS policy builder | `RLSPolicyBuilder` |
| `fluentRls(userIdColumn:usersTableName:roleColumn:)` | Get new fluent RLS policy builder | `FluentRLSPolicyBuilder` |
| `generateRLSSQL()` | Generate RLS SQL | `String` |

**RLS Builder Parameters:**
- `userIdColumn`: Column name for user ID (default: `"user_id"`)
- `usersTableName`: Name of users table (default: `"public.user_public"` for fluent API, `"users"` for legacy)
- `roleColumn`: Column name for role in users table (default: `"role"`) - Used for RBAC
- `isOnlineColumn`: Column name for online status (default: `"is_online"`) - Legacy API only
...
| Pattern | New Fluent API | Legacy API | Use Case |
|---------|----------------|------------|----------|
| User-specific data | `.who([.authenticated]).access([.read, .write, .update, .delete]).own()` | `canAccessOwn()` | Personal notes, settings |
| Public read-only | `.who([.anonymous, .authenticated]).access([.read]).match("true")` | `canAccessAll()` | Public posts, announcements |
| Social content | Combine read-all + modify-own policies | `canReadAllModifyOwnSeparate()` | Posts, comments, likes |
| Team data | `.match("team_id IN (SELECT ...)")` | `custom()` with team check | Team documents, channels |
| Admin only | `.who([.admin]).access([.read, .write, .update, .delete]).match("true")` | `admin()` or `hasRole()` | Admin settings, logs |
| Time-based | `.match("expires_at > NOW()")` | `custom()` with date check | Temporary access, expiring content |
| Multiple roles | `.who([.admin, .superadmin])` | `hasRole([...])` | Multi-role access control |
